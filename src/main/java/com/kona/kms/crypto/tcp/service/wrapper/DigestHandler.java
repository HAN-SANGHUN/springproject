...
import com.kona.kms.crypto.tcp.model.MSGDigestModel;
...
public class DigestHandler implements HandlerInterface{
	...
	synchronized public OutputModel	processRequest(MSGHeaderModel msgHeaderModel, ByteBuffer msgBodyBuffer) throws KmsException{
		...
		MSGDigestModel msgDigestModel = new MSGDigestModel(msgBodyBuffer);
		...
		if(msgDigestModel.getMechParaString() == null){
			mechParaArray = null;
		} else{
			mechParaString = msgDigestModel.getMechParaString();
			mechParaArray[0] = mechParaString;
		}
		
		inputData 	= msgDigestModel.getOutputDataTo();
		
		try{
			outputData = kmsCryptoService.C_Digest(msgDigestModel.getMechanismString(), mechParaArray, inputData);
...
//		outputModel.setTotalLen(msgHeaderModel.getTotalLenString().getBytes());
		...
		outputModel.setOutputType(msgDigestModel.getOutputType());
