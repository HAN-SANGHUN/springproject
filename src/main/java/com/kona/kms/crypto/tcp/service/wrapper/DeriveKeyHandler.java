...
import com.kona.kms.crypto.tcp.model.MSGDeriveKeyModel;
...
public class DeriveKeyHandler implements HandlerInterface {
	private static final Logger logger = LoggerFactory.getLogger(DeriveKeyHandler.class);
	...
	synchronized public OutputModel processRequest(MSGHeaderModel msgHeaderModel, ByteBuffer msgBodyBuffer) throws KmsException{
		...
		MSGDeriveKeyModel msgDeriveKeyModel = new MSGDeriveKeyModel(msgBodyBuffer);
		KmsTCPCryptoService kmsCryptoService = new KmsTCPCryptoServiceImpl();
		
		keyAttributes = msgDeriveKeyModel.getKeyAttributes();

		String[] mechParaArray = new String[1];
		
		if(msgDeriveKeyModel.getMechParaString() == null){
			mechParaArray = null;
		} else{
			String mechParaString = msgDeriveKeyModel.getMechParaString();
			mechParaArray[0] = mechParaString;
		}
	
		
		try{
			keyAttributesOut = kmsCryptoService.DeriveKey(msgDeriveKeyModel.getMechanismString(), mechParaArray, msgDeriveKeyModel.getBaseKeyLabelString(), keyAttributes);
		...
		String keyType = msgDeriveKeyModel.getInputTypeString();
		...
		}else if(keyType.equalsIgnoreCase("U")){
			GenPubKeyAttrisModel genKeyAttrisModel = new GenPubKeyAttrisModel(keyAttributesOut);
			keyAttributesArray = genKeyAttrisModel.getKeyAttrisArray();
			keyAttributesArrayLenInt = genKeyAttrisModel.getKeyAttrisLen();
...
//		outputModel.setTotalLen(msgHeaderModel.getTotalLenString().getBytes());
		...
		outputModel.setOutputType(msgDeriveKeyModel.getOutputType());
