...
public class GenerateKeyHandler implements HandlerInterface {
	private static final Logger logger = LoggerFactory.getLogger(GenerateKeyHandler.class);
	...
	synchronized public OutputModel processRequest(MSGHeaderModel msgHeaderModel, ByteBuffer msgBodyBuffer) throws KmsException{
		...
		MSGGenerateKeyModel msgGenerateKeyModel = new MSGGenerateKeyModel(msgBodyBuffer);
		KmsTCPCryptoService kmsCryptoService = new KmsTCPCryptoServiceImpl();
		
		keyAttributes = msgGenerateKeyModel.getKeyAttributes();
		
		try{
			keyAttributesOut = kmsCryptoService.GenerateKey(msgGenerateKeyModel.getTokenLabelString(), msgGenerateKeyModel.getMechanismString(), keyAttributes);
...
//		outputModel.setTotalLen(msgHeaderModel.getTotalLenString().getBytes());
		...
		outputModel.setOutputType(msgGenerateKeyModel.getOutputType());
