...
import com.kona.kms.crypto.tcp.keyAttrisModel.ImportPubKeyAttrisModel;
import com.kona.kms.crypto.tcp.model.MSGHeaderModel;
import com.kona.kms.crypto.tcp.model.MSGImportPubKeyModel;
...
public class ImportPubKeyHandler implements HandlerInterface {
	private static final Logger logger = LoggerFactory.getLogger(ImportPubKeyHandler.class);
	...
	synchronized public OutputModel processRequest(MSGHeaderModel msgHeaderModel, ByteBuffer msgBodyBuffer) throws KmsException{
		...
		MSGImportPubKeyModel msgImportPubKeyModel = new MSGImportPubKeyModel(msgBodyBuffer);
		KmsTCPCryptoService kmsCryptoService = new KmsTCPCryptoServiceImpl();
		
		keyAttributes = msgImportPubKeyModel.getKeyAttributes();
		
		
		try{
			keyAttributesOut = kmsCryptoService.ImportPublicKey(msgImportPubKeyModel.getTokenLabelString(), msgImportPubKeyModel.getMechanismString(), keyAttributes);
		...
		ImportPubKeyAttrisModel importPubKeyAttrisModel = new ImportPubKeyAttrisModel(keyAttributesOut);
		
		keyAttributesArray = importPubKeyAttrisModel.getKeyAttrisArray();
		keyAttributesArrayLenInt = importPubKeyAttrisModel.getKeyAttrisLen();
...
//		outputModel.setTotalLen(msgHeaderModel.getTotalLenString().getBytes());
		...
		outputModel.setOutputType(msgImportPubKeyModel.getOutputType());
