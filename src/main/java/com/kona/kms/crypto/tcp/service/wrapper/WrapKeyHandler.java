...
import com.kona.kms.crypto.tcp.model.MSGWrapKeyModel;
...
public class WrapKeyHandler implements HandlerInterface {
	private static final Logger logger = LoggerFactory.getLogger(WrapKeyHandler.class);
	...
	synchronized public OutputModel processRequest(MSGHeaderModel msgHeaderModel, ByteBuffer msgBodyBuffer) throws KmsException{
		...
		MSGWrapKeyModel msgWrapKeyModel = new MSGWrapKeyModel(msgBodyBuffer);
		KmsTCPCryptoService kmsCryptoService = new KmsTCPCryptoServiceImpl();
		
		if(msgWrapKeyModel.getMechParaString() == null){
			mechParaArray = null;
		} else{
			mechParaString = msgWrapKeyModel.getMechParaString();
			mechParaArray[0] = mechParaString;
		}
				
		try{
			outputData = kmsCryptoService.C_WrapKey(msgWrapKeyModel.getMechanismString(), mechParaArray, msgWrapKeyModel.getKeyLabelString(), msgWrapKeyModel.getWrapKeyLabelString());
...
//		outputModel.setTotalLen(msgHeaderModel.getTotalLenString().getBytes());
		...
		outputModel.setOutputType(msgWrapKeyModel.getOutputType());
