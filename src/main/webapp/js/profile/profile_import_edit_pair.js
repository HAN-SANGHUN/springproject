...
function validator_init(){
	...
	$('#form2').validate({
		rules:{
			...
			keyLabel1: {required:true},
			keyProfileName1: {required:true},
			description1: {required:true},
			keyProfileVersion1: {required:true}
		},
		...
		submitHandler:function(form){
			...
			if($("#keyProfileVersion").val() != $("#keyProfileVersion1").val()){
				dialog_message("Warning", $.i18n.prop("key_profile_version_should_equal"));
				$("#keyProfileVersion1").focus();
...
function doSave(callback){
	$.ajax({
		url: '/KeyProfile/popup/importPairProfile.do',
		type: 'POST',
...
function doCancel(){
	$.ajax({
		...
		type: 'POST',
		data: {'tokenID':$("#tokenID").val(), 'pubTokenID':$("#pubTokenID").val()},
		dataType: 'json',
		success: function(response){
			if(response.status == "SUCCESS"){			
