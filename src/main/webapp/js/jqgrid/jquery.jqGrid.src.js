...
//Editing
	editRow : function(rowid,keys,oneditfunc,successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		return this.each(function(){
			...
			if (editable === "0" && !$(ind).hasClass("not-editable-row")) {
				...
				$(ind).addClass("ui-state-highlight");		//20130819 - piki
			}
		});
	},
	saveRow : function(rowid, successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		if (editable==="1") {
			...
			$(ind).removeClass("ui-state-highlight");		//20130819 - piki
		}
		return success;
	},
...
$.jgrid.extend({
    ...
    columnChooser : function(opts) {
    	//2013-09-02 - multi select를 checkbox로 변경 - piki
        var self = this;
		if($("#colchooser_"+$.jgrid.jqID(self[0].p.id)).length ) { return; }
        var selector = $('<div id="colchooser_'+self[0].p.id+'" style="position:relative;overflow:hidden"><input type="checkbox" id="sp_multiple_all" >Select All<div style="overflow-y: auto;"><span id="sp_multiple"></span></div></div>');
        var select = $('#sp_multiple', selector);
        
        var select_all = $('#sp_multiple_all',selector);
        select_all.on('click', function(event){
        	if($(this).attr('checked') == 'checked'){
        		$('[type=checkbox]',select).attr('checked', 'checked');
        	}else{
        		$('[type=checkbox]',select).removeAttr('checked');
        	}
        });
        ...
        opts = $.extend({
            ...
            "apply_perm" : function() {
                ...
                var perm = [];
				//fixedCols.slice(0);
                $('[type=checkbox]:checked',select).each(function() { perm.push(parseInt(this.value,10)); });
        ...
        $.each(colModel, function(i) {
            ...
            select.append("<input type='checkbox' value='"+i+"' "+(this.hidden?"":"checked='checked'")+" />"+$.jgrid.stripHtml(colNames[i])+"<br/>");
        });
